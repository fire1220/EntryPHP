...
class DbMysqli{
    ...
    private $lastsql;
    private $connectRead;
    private $connectWrite;
    private $linkserver;
    private $linkserverRead;
    private $linkserverWrite;
    private $singleton;
    public function __construct($table,$linkserver,$singleton=true){
        $this->table = $table;
        if(isset($linkserver['dbRead'])&&isset($linkserver['dbWrite'])){
            $this->linkserverRead = $linkserver['dbRead'];
            $this->linkserverWrite = $linkserver['dbWrite'];
        }else{
            $this->linkserverRead = $linkserver;
            $this->linkserverWrite = $linkserver;
        }
        $this->singleton = $singleton;
    }
    public function test(){
        echo '<pre>Read';
        print_r($this->connectRead);
        echo '</pre>';
        echo '<pre>Write';
        print_r($this->connectWrite);
        echo '</pre>';
    }
    private function mysqlQuery($sql){
        $this->connectRead = connectDb::init($this->linkserverRead,$this->singleton);
        $source = mysqli_query($this->connectRead,$sql);
    ...
    private function mysqlExecute($sql){
        $this->connectWrite = connectDb::init($this->linkserverWrite,$this->singleton);
        $source = mysqli_query($this->connectWrite,$sql);
        $this->responseSource($source);
        return $source;
    }
    private function mysqlInsertId(){
        return mysqli_insert_id($this->connectWrite);
    }
    private function mysqlAffectedRows(){
        return mysqli_affected_rows($this->connectWrite);
    }
    ...
    public function add(){
        ...
        $source = $this->mysqlExecute($insert_sql);
        $user_insert_id = $this->mysqlInsertId();
        $this->clearopj();
    ...
    public function save(){
        if(!empty($this->where)){
            ...
            $affected = $this->mysqlAffectedRows();
            $this->clearopj();
            return $affected;
        }
    }
